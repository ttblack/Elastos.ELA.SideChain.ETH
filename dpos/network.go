...
import (
	...
	"github.com/elastos/Elastos.ELA.SideChain.ETH/dpos/p2p/msg"
...
func (n *network) processMessage(msgItem *messageItem) {
	...
	//	//}
	//case dposp2p.CmdBlock:
	//	//msgBlock, processed := m.(*msg.Block)
	//	//if processed {
...
func (n *network) confirmReceived(p *payload.Confirm, height uint64) {
...
func NewDposNetwork(cfg NetworkConfig) (*network, error) {
	...
	var pid peer.PID
	copy(pid[:], cfg.Account.Accounts()[0].Address.Bytes())
	server, err := p2p.NewServer(&p2p.Config{
		DataDir:          dataPathDPoS,
		PID:              pid,
		EnableHub:        true,
		Localhost:        cfg.ChainParams.IPAddress,
		MagicNumber:      cfg.ChainParams.Magic,
		DefaultPort:      cfg.ChainParams.DPoSPort,
