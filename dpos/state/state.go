...
import (
	...
	dcom "github.com/elastos/Elastos.ELA.SideChain.ETH/dpos/common"
...
type Producer struct {
	...
	cancelHeight           uint64
	inactiveCountingHeight uint64
	inactiveSince          uint64
	activateRequestHeight  uint64
	illegalHeight          uint64
	penalty                uint64
	votes                  uint64
	depositAmount          uint64
	depositHash            com.Uint168
}

// Info returns a copy of the origin registered producer info.
func (p *Producer) Info() dtypes.ProducerInfo {
	return p.info
}
...
func (p *Producer) Penalty() uint64 {
	return p.penalty
}

func (p *Producer) InactiveSince() uint64 {
	return p.inactiveSince
}

func (p *Producer) IllegalHeight() uint64 {
	return p.illegalHeight
}

func (p *Producer) ActivateRequestHeight() uint64 {
	return p.activateRequestHeight
}

func (p *Producer) DepositAmount() uint64 {
...
func (p *Producer) Deserialize(s rlp.Stream) (err error) {
...
type State struct {
	*StateKeyFrame
	...
	getProducerDepositAmount func(programHash com.Uint168) (
		uint64, error)

	mtx     sync.RWMutex
	history *dcom.History

	votesCacheKeys map[uint64][]string
...
// will be updated.
func (s *State) updateProducerInfo(origin *dtypes.ProducerInfo, update *dtypes.ProducerInfo) {
...
func (s *State) ExistProducerByDepositHash(programHash com.Uint168) bool {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *State) ProcessBlock(block *types.Block, confirm *payload.Confirm) {
	...
	s.tryInitProducerAssetAmounts(block.NumberU64())
	s.processTransactions(block.Transactions(), block.NumberU64())
	s.ProcessVoteStatisticsBlock(block)

	if confirm != nil {
		s.countArbitratorsInactivity(block.NumberU64(), confirm)
	}

	// Commit changes here if no errors found.
	s.history.Commit(block.NumberU64())
}
...
func (s *State) processTransactions(txs []*types.Transaction, height uint64) {
...
// content.
func (s *State) processTransaction(tx *types.Transaction, height uint64) {
	//switch tx.TxType {
...
func (s *State) registerProducer(tx *types.Transaction, height uint64) {
...
func (s *State) updateProducer(info *dtypes.ProducerInfo, height uint64) {
...
func (s *State) cancelProducer(payload *payload.ProcessProducer, height uint64) {
...
func (s *State) activateProducer(p *payload.ActivateProducer, height uint64) {
...
func (s *State) processVotes(tx *types.Transaction, height uint64) {
	//if tx.Version >= types.TxVersion09 {
	//	// Votes to producers.
	//	for i, output := range tx.Outputs {
	//		if output.Type != types.OTVote {
...
func (s *State) tryInitProducerAssetAmounts(blockHeight uint64) {
	//if blockHeight != s.chainParams.CRVotingStartHeight {
	//	return
	//}

	//setAmount := func(producer *Producer, amount uint64) {
	//	s.history.Append(blockHeight, func() {
	//		producer.depositAmount = amount
	//	}, func() {
	//		producer.depositAmount = 0
	//	})
	//}
...
func (s *State) processDeposit(tx *types.Transaction, height uint64) {
	//for i, output := range tx.Outputs {
...
func (s *State) getProducerByDepositHash(hash com.Uint168) *Producer {
	for _, producer := range s.PendingProducers {
...
//func (s *State) addProducerAssert(output *types.Output, height uint64) bool {
...
func (s *State) processCancelVotes(tx *types.Transaction, height uint64) {
	//for _, input := range tx.Inputs {
...
//func (s *State) processVoteOutput(output *types.Output, height uint64) {
	...
	//}
	//
	//countByVote := func(producer *Producer, vote uint64) {
	//	s.history.Append(height, func() {
	//		producer.votes += vote
	//	}, func() {
	//		producer.votes -= vote
...
//func (s *State) processVoteCancel(output *types.Output, height uint64) {
...
func (s *State) returnDeposit(tx *types.Transaction, height uint64) {
...
func (s *State) updateVersion(tx *types.Transaction, height uint64) {
...
func (s *State) processEmergencyInactiveArbitrators(
	inactivePayload *payload.InactiveArbitrators, height uint64) {
...
//func (s *State) ProcessSpecialTxPayload(p types.Payload, height uint64) {
//	s.mtx.Lock()
//	defer s.mtx.Unlock()
//
...
func (s *State) countArbitratorsInactivity(height uint64,
...
func (s *State) tryRevertInactivity(key string, producer *Producer,
	needReset bool, height, startHeight uint64) {
	if needReset {
...
func (s *State) RollbackTo(height uint64) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *State) GetHistory(height uint64) (*StateKeyFrame, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func NewState(getArbiters func() [][]byte) *State {
	return &State{
		getArbiters:              getArbiters,
		history:                  dcom.NewHistory(maxHistoryCapacity),
		StateKeyFrame:            NewStateKeyFrame(),
		votesCacheKeys:           make(map[uint64][]string),
