...
type degradation struct {
	...
	understaffedSince uint64
	inactivateHeight  uint64
	inactiveTxs       map[common.Hash]interface{}
}
...
func (d *degradation) RollbackTo(height uint64) {
	d.mtx.Lock()
...
func (d *degradation) InactiveModeSwitch(height uint64,
...
func (d *degradation) TrySetUnderstaffed(height uint64) bool {
	d.mtx.Lock()
	if d.state != DSNormal {
		d.mtx.Unlock()
		return false
...
func (d *degradation) AddInactivePayload(p *types.InactiveArbitrators) bool {
	hash := p.Hash()
	d.mtx.Lock()
