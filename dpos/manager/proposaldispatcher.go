...
type ProposalDispatcherConfig struct {
	...
	EventMonitor *dlog.EventMonitor
	Consensus    *Consensus
	Network      DPOSNetwork
	Manager      *DPOSManager
...
type ProposalDispatcher struct {
	...
	finishedBlockHash   com.Uint256
	processingBlock     *types.Block
	processingProposal  *payload.DPOSProposal
	acceptVotes         map[com.Uint256]*payload.DPOSProposalVote
	rejectedVotes       map[com.Uint256]*payload.DPOSProposalVote
	pendingProposals    map[com.Uint256]*payload.DPOSProposal
	precociousProposals map[com.Uint256]*payload.DPOSProposal
	pendingVotes        map[com.Uint256]*payload.DPOSProposalVote
	...
	signedTxs                   map[common.Hash]interface{}
...
func (p *ProposalDispatcher) RequestAbnormalRecovering() {
	...
	//msgItem := &msg.RequestConsensus{Height: height}
...
func (p *ProposalDispatcher) GetFinishedHeight() uint64 {
...
func (p *ProposalDispatcher) GetFinishedBlockHash() com.Uint256 {
...
func (p *ProposalDispatcher) StartProposal(b *types.Block) {
	...
	//p.processingBlock = b
	//
	//proposal := &dtypes.DPOSProposal{Sponsor: p.cfg.Manager.GetPublicKey(),
	...
	//}
	//
	//log.Info("[StartProposal] send proposal message finished, Proposal Hash: ", msg.GetMessageHash(m))
	//p.cfg.Network.BroadcastMessage(m)
	//
	//proposalEvent := dlog.ProposalEvent{
	//	Sponsor:      common.Bytes2Hex(proposal.Sponsor),
...
func (p *ProposalDispatcher) CleanProposals(changeView bool) {
	...
	p.acceptVotes = make(map[com.Uint256]*payload.DPOSProposalVote)
	p.rejectedVotes = make(map[com.Uint256]*payload.DPOSProposalVote)
	p.pendingVotes = make(map[com.Uint256]*payload.DPOSProposalVote)
	p.proposalProcessFinished = false
	if !changeView {
		p.inactiveCountDown.Reset(0)
		p.currentInactiveArbitratorTx = nil
		p.signedTxs = map[common.Hash]interface{}{}
		p.pendingProposals = make(map[com.Uint256]*payload.DPOSProposal)
		p.precociousProposals = make(map[com.Uint256]*payload.DPOSProposal)
...
func (p *ProposalDispatcher) OnBlockAdded(b *types.Block) {
	for k, v := range p.pendingProposals {
		if p.cfg.Consensus.IsRunning() && bytes.Equal(v.BlockHash.Bytes(), b.Hash().Bytes()) {
...
func (p *ProposalDispatcher) FinishConsensus(height uint64, blockHash com.Uint256) {
	if p.cfg.Consensus.IsRunning() {
		...
		c := dlog.ConsensusEvent{EndTime: p.cfg.TimeSource.AdjustedTime(), Height: height}
...
func (p *ProposalDispatcher) CollectConsensusStatus(status *msg.ConsensusStatus) error {
...
func (p *ProposalDispatcher) RecoverFromConsensusStatus(status *msg.ConsensusStatus) error {
	p.acceptVotes = make(map[com.Uint256]*payload.DPOSProposalVote)
...
func (p *ProposalDispatcher) checkInactivePayloadContent(
	inactivePayload *dtypes.InactiveArbitrators) error {
	...
	for _, v := range inactivePayload.Arbitrators {
		if _, exist := inactiveArbitratorsMap[common.Bytes2Hex(
			v)]; !exist {
...
func (p *ProposalDispatcher) OnResponseInactiveArbitratorsReceived(
	txHash *common.Hash, signer []byte, sign []byte) {
	log.Info("[OnResponseInactiveArbitratorsReceived] collect transaction" +
		" signs")

	if p.currentInactiveArbitratorTx == nil ||
		!bytes.Equal(p.currentInactiveArbitratorTx.Hash().Bytes(), txHash.Bytes()) {
...
func (p *ProposalDispatcher) acceptProposal(d *payload.DPOSProposal) {
	...
	//vote := &dtypes.DPOSProposalVote{ProposalHash: d.Hash(),
	...
	//log.Info("[acceptProposal] send acc_vote msg:", msg.GetMessageHash(voteMsg).String())
	//
	//voteEvent := dlog.VoteEvent{Signer: common.BytesToHexString(vote.Signer),
...
func (p *ProposalDispatcher) rejectProposal(d *payload.DPOSProposal) {
	...
	//	return
	//}
	//voteMsg := &msg.Vote{Command: msg.CmdRejectVote, Vote: *vote}
	//log.Info("[rejectProposal] send rej_vote msg:", msg.GetMessageHash(voteMsg))
	//
	...
	//
	//voteEvent := dlog.VoteEvent{Signer: common.Bytes2Hex(vote.Signer),
...
func (p *ProposalDispatcher) setProcessingProposal(d *payload.DPOSProposal) (finished bool) {
	...
	for _, v := range p.pendingVotes {
		if bytes.Equal(v.ProposalHash.Bytes(), d.Hash().Bytes()) {
...
func NewDispatcherAndIllegalMonitor(cfg ProposalDispatcherConfig) (
	*ProposalDispatcher, *IllegalBehaviorMonitor) {
	p := &ProposalDispatcher{
		...
		acceptVotes:            make(map[com.Uint256]*payload.DPOSProposalVote),
		rejectedVotes:          make(map[com.Uint256]*payload.DPOSProposalVote),
		pendingProposals:       make(map[com.Uint256]*payload.DPOSProposal),
		precociousProposals:    make(map[com.Uint256]*payload.DPOSProposal),
		pendingVotes:           make(map[com.Uint256]*payload.DPOSProposalVote),
		signedTxs:              make(map[common.Hash]interface{}),
	...
	i := &IllegalBehaviorMonitor{
		dispatcher:      p,
		cachedProposals: make(map[com.Uint256]*payload.DPOSProposal),
		evidenceCache: evidenceCache{
			make(map[com.Uint256]payload.DPOSIllegalData)},
		manager: cfg.Manager,
	}
	p.illegalMonitor = i
	return p, i
