...
type IllegalBehaviorMonitor struct {
	...
	cachedProposals map[com.Uint256]*payload.DPOSProposal

	evidenceCache evidenceCache
	manager       *DPOSManager

	inactiveArbitratorsPayloadHash *com.Uint256
}
...
func (i *IllegalBehaviorMonitor) SetInactiveArbitratorsTxHash(
	hash common.Hash) {
	phash := com.Uint256{}
	copy(phash[:], hash.Bytes()[:])
	i.inactiveArbitratorsPayloadHash = &phash
}
...
func (i *IllegalBehaviorMonitor) Reset(changeView bool) {
	i.cachedProposals = make(map[com.Uint256]*payload.DPOSProposal)
...
func (i *IllegalBehaviorMonitor) IsLegalProposal(p *dtype.DPOSProposal) (*dtype.DPOSProposal, bool) {
...
func (i *IllegalBehaviorMonitor) generateProposalEvidence(
	...
	return &payload.ProposalEvidence{
		...
		BlockHeight: uint32(block.NumberU64()),
	}, nil
}
...
func (i *IllegalBehaviorMonitor) ProcessIllegalProposal(
	...
	asc := true
	if bytes.Compare(first.Hash().Bytes(), second.Hash().Bytes()) > 0 {
		asc = false
	}
...
//	evidence *dtype.SidechainIllegalData) {
	//tx := &types.Transaction{
	//	Version:        types.TxVersion09,
...
func (i *IllegalBehaviorMonitor) isProposalsIllegal(first, second *payload.DPOSProposal) bool {
	...
	if bytes.Equal(first.BlockHash.Bytes(), second.BlockHash.Bytes()) {
		return false
	}
	...
	if firstBlock.NumberU64() == secondBlock.NumberU64() {
		return true
	}
...
func (i *IllegalBehaviorMonitor) isVotesIllegal(first, second *payload.DPOSProposalVote) bool {
	...
	if bytes.Equal(first.ProposalHash.Bytes(), second.ProposalHash.Bytes()) {
		return !bytes.Equal(first.Hash().Bytes(), second.Hash().Bytes())
	}
