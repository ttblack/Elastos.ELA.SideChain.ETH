...
func (s *DposStore) getHeights() ([]uint64, error) {
	...
	heights := make([]uint64, 0, count+1)
	var h uint64
	for i := uint64(0); i < count; i++ {
		if h, err = common.ReadUint64(r); err != nil {
			return nil, err
		}
...
func (s *DposStore) appendHeights(batch Batch, height uint64) error {
	heights, err := s.getHeights()
	if err != nil {
		return err
	}
	heights = append(heights, height)
	heightSet := make(map[uint64]interface{})
	...
	for h := range heightSet {
		if err = common.WriteUint64(buf, h); err != nil {
			return err
		}
	}
...
func (s *DposStore) getFlatCheckPoint(height uint64) (*state.CheckPoint,
	error) {
