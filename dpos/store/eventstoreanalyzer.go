...
type EventStoreAnalyzer struct {
	...
	viewCount  uint32
	voteEvents map[com.Uint256]string // vote hash as key, signer as value
}
...
func (e *EventStoreAnalyzer) AppendConsensusVote(vote *payload.DPOSProposalVote) {
	if vote != nil {
		e.voteEvents[vote.Hash()] = common.Bytes2Hex(vote.Signer)
	}
}

func (e *EventStoreAnalyzer) Clear() {
	e.viewCount = 0
	e.voteEvents = map[com.Uint256]string{}
}
...
func NewEventStoreAnalyzer(cfg EventStoreAnalyzerConfig) *EventStoreAnalyzer {
	return &EventStoreAnalyzer{
		cfg:        cfg,
		viewCount:  0,
		voteEvents: map[com.Uint256]string{},
	}
}
