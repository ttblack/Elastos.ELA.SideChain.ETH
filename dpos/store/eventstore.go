...
import (
	...
	elog "github.com/elastos/Elastos.ELA/dpos/log"
)
...
type addVoteEventTask struct {
	event *elog.VoteEvent
	reply chan bool
}

type addViewEventTask struct {
	event *elog.ViewEvent
	reply chan bool
}
	...
	for {
		...
		case t := <-s.eventCh:
			...
			case *addConsensusEventTask:
				...
				log.Debug("handle add consensus event task exetime: %g", tcall)
			case *updateConsensusEventTask:
				s.handleUpdateConsensusEvent(task.event)
				task.reply <- true
				tcall := float64(time.Now().Sub(now)) / float64(time.Second)
				log.Debug("handle update consensus event task exetime: %g", tcall)
			case *addProposalEventTask:
				s.handleAddProposalEvent(task.event)
				task.reply <- true
				tcall := float64(time.Now().Sub(now)) / float64(time.Second)
				log.Debug("handle add proposal event task exetime: %g", tcall)
			case *updateProposalEventTask:
				s.handleUpdateProposalEvent(task.event)
				task.reply <- true
				tcall := float64(time.Now().Sub(now)) / float64(time.Second)
				log.Debug("handle update proposal event task exetime: %g", tcall)
			case *addVoteEventTask:
				s.handleVoteProposalEvent(task.event)
				task.reply <- true
				tcall := float64(time.Now().Sub(now)) / float64(time.Second)
				log.Debug("handle add vote event task exetime: %g", tcall)
			case *addViewEventTask:
				s.handleViewProposalEvent(task.event)
				task.reply <- true
				tcall := float64(time.Now().Sub(now)) / float64(time.Second)
				log.Debug("handle add view event task exetime: %g", tcall)
			}
...
func (s *DposStore) handleAddConsensusEvent(cons *elog.ConsensusEvent) {
...
func (s *DposStore) handleUpdateConsensusEvent(cons *elog.ConsensusEvent) {
...
func (s *DposStore) handleAddProposalEvent(prop *elog.ProposalEvent) {
...
func (s *DposStore) handleUpdateProposalEvent(prop *elog.ProposalEvent) {
...
func (s *DposStore) handleVoteProposalEvent(vote *elog.VoteEvent) {
...
func (s *DposStore) handleViewProposalEvent(view *elog.ViewEvent) {
...
func (s *DposStore) AddConsensusEvent(event interface{}) error {
	e, ok := event.(*elog.ConsensusEvent)
	if !ok {
...
func (s *DposStore) addConsensusEvent(cons *elog.ConsensusEvent) (uint64, error) {
...
func (s *DposStore) updateConsensusEvent(cons *elog.ConsensusEvent) ([]uint64, error) {
...
func (s *DposStore) addProposalEvent(event *elog.ProposalEvent) (uint64, error) {
	buf := new(bytes.Buffer)
...
func (s *DposStore) updateProposalEvent(event *elog.ProposalEvent) ([]uint64, error) {
...
func (s *DposStore) addVoteEvent(event *elog.VoteEvent) (uint64, error) {
	vote := event.RawData
...
func (s *DposStore) addViewEvent(event *elog.ViewEvent) (uint64, error) {
