...
func mockRemotePeer(pid peer.PID, priKey []byte, port uint16,
	...
	cfg := &peer.Config{
		PID:          pid,
		Magic:        123123,
		PingInterval: p2p2.defaultPingInterval,
...
func mockInboundPeer(addr p2p2.PeerAddr, priKey []byte, pc chan<- *peer.Peer,
...
func TestServerConnections(t *testing.T) {
	...
	servers := 71
	cfgs := make([]p2p2.Config, 0, servers)
...
func TestServer_ConnectPeers(t *testing.T) {
	...
	addrList := make(map[peer.PID]string)
	server, err := NewServer(&p2p2.Config{
		PID:         pid,
		MagicNumber: 123123,
		DefaultPort: 20338,
		TimeSource:  dtime.NewMedianTime(),
...
func TestServer_DumpPeersInfo(t *testing.T) {
	...
	for _, p := range peers {
		switch p.State {
		case p2p2.CS2WayConnection:
			connPeers++
		case p2p2.CSOutboundOnly:
			outPeers++
		case p2p2.CSNoneConnection:
			nonePeers++
		}
	}
