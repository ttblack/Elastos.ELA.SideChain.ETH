...
import (
	...
	"github.com/elastos/Elastos.ELA.SideChain.ETH/dpos/p2p/addrmgr"
...
out:
	for {
		...
		case <-idleTimer.C:
			log.Warn("pipe no response for %s -- timeout", pipeTimeout)
			break out
		}
	}
...
func (h *Hub) handleOutbound(state *state, conn *Conn) {
	...
	if _, ok := state.peers[PIDTo16(conn.PID())]; !ok {
		log.Debug("%s not in peers list", peer.PID(conn.PID()))
		_ = conn.Close()
		return
	}
	...
	if !ok {
		log.Debug("target %s not in peers list", target)
		_ = conn.Close()
		return
	}
	...
	if addr == nil {
		log.Debug("target %s address not found", target)
		_ = conn.Close()
		return
	}
...
func (h *Hub) handleInbound(state *state, conn *Conn) {
	// Refuse connection not in connect peers list.
	if _, ok := state.peers[PIDTo16(conn.PID())]; !ok {
		log.Debug("%x not in peers list", conn.PID())
		_ = conn.Close()
		return
	}
	...
	if !ok {
		log.Debug("service magic %d not found", conn.Magic())
		_ = conn.Close()
		return
	}
...
func (h *Hub) Intercept(conn net.Conn) net.Conn {
	c, err := WrapConn(conn)
	if err != nil {
		log.Error("intercept connection failed, %s", err)
		_ = conn.Close()
		return nil
	}
