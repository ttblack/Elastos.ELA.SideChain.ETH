...
type Config struct {
	...
	Sign 			 func(account accounts.Account, text []byte) ([]byte, error)
...
func newNetAddress(addr net.Addr, services uint64) (*dposp2p.NetAddress, error) {
	// addr will be a net.TCPAddr when not using a proxy.
	if tcpAddr, ok := addr.(*net.TCPAddr); ok {
		ip := tcpAddr.IP
		port := uint16(tcpAddr.Port)
		na := dposp2p.NewNetAddressIPPort(ip, port, services)
		return na, nil
	}
	...
	na := dposp2p.NewNetAddressIPPort(ip, uint16(port), services)
	return na, nil
}

// HostToNetAddrFunc is a func which takes a host, port, services and returns the netaddress.
type HostToNetAddrFunc func(host string, port uint16, services uint64) (*dposp2p.NetAddress, error)
...
//
// This function is safe for concurrent access.
func (p *Peer) NA() *dposp2p.NetAddress {
	p.flagsMtx.Lock()
	na := p.na
	p.flagsMtx.Unlock()
...
func (p *Peer) inHandler() {
	...
	idleTimer := time.AfterFunc(idleTimeout, func() {
		...
		log.Warn("Peer %s no answer for %s -- disconnecting", p, idleTimeout)
		p.Disconnect()
	})
...
out:
	for atomic.LoadInt32(&p.disconnect) == 0 {
		...
		case *msg.VerAck:
			...
			log.Info("Already received 'verack' from peer %v -- disconnecting", p)
			break out
...
func RandomUint64() uint64 {
	var randNum uint64
	_ = binary.Read(rand.Reader, binary.BigEndian, &randNum)
	return randNum
}
...
func NewOutboundPeer(cfg *Config, addr string) (*Peer, error) {
	...
	p.na = dposp2p.NewNetAddressIPPort(net.ParseIP(host), uint16(port), 0)
