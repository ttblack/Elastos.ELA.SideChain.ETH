...
import (
	...
	"github.com/elastos/Elastos.ELA.SideChain.ETH/dpos/p2p/connmgr"
	"github.com/elastos/Elastos.ELA.SideChain.ETH/dpos/p2p/hub"
...
func (sp *serverPeer) OnVersion(_ *peer.Peer, v *msg.Version) {
	...
	if sp.connReq != nil && !bytes.Equal(v.PID[:], sp.connReq.PID[:]) {
		log.Info("Disconnecting peer %v - version PID not match", sp)
		sp.Disconnect()
		return
	}
...
func (s *server) handleAddPeerMsg(state *peerState, sp *serverPeer) bool {
	...
	if _, ok := state.connectPeers[sp.PID()]; !ok {
		log.Info("New peer %s ignored - not in connect list", sp)
		sp.Disconnect()
		return false
	}
...
func (s *server) dialTimeout(addr net.Addr) (net.Conn, error) {
	log.Debug("Server dial addr %s", addr)
