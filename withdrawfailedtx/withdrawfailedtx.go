...
func FailedWithrawInit(datadir string, evtMux *event.TypeMux) {
	db, err := leveldb.New(filepath.Join(datadir, "failedwithdraw.db"), 768, 16, "eth/db/ela/")
	if err != nil {
...
func ReceivedFailedWithdrawTx(hash string, signature string) error {
	fmt.Println("ReceivedFailedWithdrawTx 111111")
	mulFailedMux.Lock()
	...
	if IsSignatureVerifyed(hash, signature) {
	...
	verifyedSigList := failedTxList[hash]

	if len(verifyedSigList) >= getMaxArbitersSign(len(arbiters)) {
		return errors.New("all ready received 2/3 signatures")
	}

	if verifySignature(arbiters, hash, signature) {
		verifyedSigList = append(verifyedSigList, signature)
		failedTxList[hash] = verifyedSigList
		if len(verifyedSigList) >= getMaxArbitersSign(len(arbiters)) {
			putFailedTxToDB(hash)
...
func getTxData(txid common.Hash) ([]byte, error) {
	...
	for _, sig := range signatures {
		...
		sigdata = common.LeftPadBytes(sigdata, 64)
...
func VerifySignatures(input []byte) bool {
	...
	count := 0
	for _, sig := range signatures {
		if verifySignature(arbiters, failedTxHash.String(), sig) == false {
			return false
		}
		count ++
	}
...
func verifySignature(arbiters []string, txid, signature string) bool {
...
func IsSignatureVerifyed(txid, signature string) bool {
...
func putFailedTxToDB(txid string) {
	list := failedTxList[txid]
	if len(list) <= 0 {
		return
	}
	buff := new(bytes.Buffer)
	elacom.WriteUint32(buff, uint32(len(list)))
	for _, sig := range list {
		elacom.WriteVarString(buff, sig)
	}

	key := FailedTxPre + txid
	failedTxdb.Put([]byte(key), buff.Bytes())
}
...
func getFailedTx(txid string) ([]string, error) {
	...
	if failedTxdb == nil {
		return list, errors.New("failed db is not init")
	}
	key := FailedTxPre + txid
	data, err := failedTxdb.Get([]byte(key))
	if err != nil {
		log.Error("GetFailedTx", "error", err)
		return list, errors.New(ErrNoSignature)
	}
	list = make([]string, 0)
	buff := bytes.NewBuffer(data)
	count, err := elacom.ReadUint32(buff)
	for i := 0; uint32(i) < count; i++ {
		sig, err := elacom.ReadVarString(buff)
		if err != nil {
			return list, err
		}
