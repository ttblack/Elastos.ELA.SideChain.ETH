...
var (
	mtx                sync.Mutex
	blockHashesCache   []common.Hash
	blockConfirmsCache []bool
	blocksCache        map[common.Hash]map[bool][]byte
...
type Serializable interface {
	...
	//read data to reader
	Deserialize(s *rlp.Stream) error
}
...
func init() {
	blockHashesCache = make([]common.Hash, 0, BlocksCacheSize)
	blockConfirmsCache = make([]bool, 0, BlocksCacheSize)
	blocksCache = make(map[common.Hash]map[bool][]byte)
}
