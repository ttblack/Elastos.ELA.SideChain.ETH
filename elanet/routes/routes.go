...
import (
	...
	crypto2 "github.com/elastos/Elastos.ELA.SideChain.ETH/crypto"
	"github.com/elastos/Elastos.ELA.SideChain.ETH/dpos/dtime"
	dp "github.com/elastos/Elastos.ELA.SideChain.ETH/dpos/p2p/peer"
...
type cache struct {
	requested map[com.Uint256]struct{}
}
...
type Config struct {
	...
	Sign func(accounts.Account, string, []byte) ([]byte,error)
...
type Routes struct {
	...
	addrIndex map[dp.PID]map[dp.PID]com.Uint256
	knownAddr map[com.Uint256]*msg.DAddr
	knownList *list.List
...
func (r *Routes) addrHandler() {
	state := &state{
		...
		requested: make(map[com.Uint256]struct{}),
	...
	for {
		...
		case <-r.announce:
			...
			for pid := range state.peers {
				...
				if err != nil {
					log.Warn("encrypt addr %s failed %s", r.addr, err)
					continue
				}
				addr := msg.DAddr{
					PID:       r.pid,
				...
				pub, err := crypto2.UnmarshalPubkey(r.pid[:])
				if err != nil {
					log.Error("UnmarshalPubkey error:", err.Error())
					return
				}
				signer := crypto2.PubkeyToAddress(*pub)
				addr.Signature, err = r.sign(accounts.Account{Address: signer}, "", addr.Data())
				if err != nil {
					log.Error("UnmarshalPubkey error:", err.Error())
					return
				}
...
func (r *Routes) handleNewPeer(s *state, p *peer.Peer) {
	...
	s.peerCache[p] = &cache{requested: make(map[com.Uint256]struct{})}
}

func (r *Routes) handleDonePeer(s *state, p *peer.Peer) {
	c, exists := s.peerCache[p]
	if !exists {
		log.Warn("Received done peer message for unknown peer %s", p)
		return
	}
...
func (r *Routes) handlePeersMsg(state *state, peers []dp.PID) {
	...
	for _, pid := range peers {
		...
		if !ok {
			r.addrMtx.Lock()
			r.addrIndex[pid] = make(map[dp.PID]com.Uint256)
			r.addrMtx.Unlock()
		}
	}
...
func (r *Routes) handleInv(s *state, p *peer.Peer, m *msg.Inv) {
	c, exists := s.peerCache[p]
	if !exists {
		log.Warn("Received inv message for unknown peer %s", p)
		return
	}
...
func (r *Routes) handleDAddr(s *state, p *peer.Peer, m *msg.DAddr) {
	c, exists := s.peerCache[p]
	if !exists {
		log.Warn("Received getdaddr message for unknown peer %s", p)
		return
	}
	...
	if _, ok := c.requested[hash]; !ok {
		log.Warn("Got unrequested addr %s from %s -- disconnecting",
			hash, p)
		p.Disconnect()
		return
	}
	...
	if err := r.verifyDAddr(s, m); err != nil {
		log.Warn("Got invalid addr %s %s from %s -- disconnecting",
			hash, err, p)
		p.Disconnect()
		return
	}
	...
	_, ok := s.peers[m.PID]
	if !ok {
		log.Debug("PID not in arbiter list")
...
func (r *Routes) OnGetData(p *peer.Peer, m *msg.GetData) {
	...
	for _, iv := range m.InvList {
		...
		case msg.InvTypeAddress:
			...
			r.addrMtx.RUnlock()
			if !ok {
				log.Warn("%s for DAddr not found", iv.Hash)
				continue
			}
...
func New(cfg *Config) *Routes {
	...
	r := Routes{
		...
		addr:      cfg.Addr,
		sign:      cfg.Sign,
		addrIndex: make(map[dp.PID]map[dp.PID]com.Uint256),
		knownAddr: make(map[com.Uint256]*msg.DAddr),
		knownList: list.New(),
